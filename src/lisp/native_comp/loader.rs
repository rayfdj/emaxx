...
pub(super) fn load(
    ...
    let loaded_units = interpreter
        .forwarded_c_value("comp-loaded-comp-units-h", environment)
        .unwrap_or(Value::Nil);
...
fn first_load(
    ...
    let expected_hash = interpreter
        .forwarded_c_value("comp-abi-hash", environment)
        .unwrap_or(Value::Nil);
    ...
    if interpreter
        .forwarded_c_value("purify-flag", environment)
...
pub(super) fn register_with_state(
    ...
    let late_pending_table = if matches!(kind, RegistrationKind::LateSubroutine) {
        let table = interpreter
            .forwarded_c_value("comp-deferred-pending-h", environment)
            .unwrap_or(Value::Nil);
