...
impl NativeRuntime {
    ...
    pub(crate) fn finish_call(&mut self, interpreter: &mut Interpreter) -> Result<(), LispError> {
        ...
        // outermost return on THIS logical thread must flush every remaining
        // typed cache before its native read barrier is no longer active.
        // Another suspended thread can still own calls in the shared heap.
...
mod tests {
    ...
    fn native_make_closure_follows_alloc_c_copy_contract() {
        ...
        let prototype_constants = Value::vector([Value::symbol("placeholder"), retained.clone()]);
    ...
    fn native_mapcar_follows_fns_c_list_branch() {
        ...
        let vector = Value::vector([Value::Integer(4), Value::Integer(5)]);
        assert_eq!(
            runtime
                .invoke(
                    &mut interpreter,
    ...
    fn native_symbol_value_and_type_of_follow_data_c() {
        ...
        for (value, expected) in [
            ...
            (
                Value::vector([Value::Integer(1), Value::Integer(2)]),
                Value::symbol("vector"),
            ),
        ] {
            assert_eq!(
