...
impl Interpreter {
    ...
    pub(crate) fn void_builtin_variable_fallback(&mut self, name: &str) {
        if self.voided_variable_fallbacks.insert(name.to_string()) {
            // A fallback-only symbol can have a cached native value even
            // when remove_global_binding had no stored binding to remove.
            self.bump_symbol_value_cell_epoch();
        }
    }
    ...
    fn update_forwarded_eval_cell(&mut self, name: &str, value: &Value) {
        ...
        // value cell.  Subsequent Lisp writes no longer alter its C slot.
        if self.detached_forwarded_variables.contains_key(name) {
            return;
        }
        match name {
    ...
    pub fn prepare_variable_assignment(
        ...
        match name {
            ...
            _ => {
                ...
                if self.initialized_forwarded_variables.get(name)
                    == Some(&crate::lisp::primitives::generated_gnu_c_forwarded_variables::ForwardedVariableKind::Bool)
                {
