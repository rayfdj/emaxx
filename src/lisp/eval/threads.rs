...
impl Interpreter {
    ...
    pub fn signal_thread(
        ...
        target.signal_condition = condition;
        target.signal_data = data;
        let target_status = target.status.clone();
        if let ThreadStatus::Blocked(ThreadBlocker::ConditionVariable(id)) = target_status {
            // thread_signal_callback broadcasts the actual wait_condvar,
            // including for a nil condition and to other waiters on it.
            for thread in &mut self.thread_states {
                if matches!(thread.status, ThreadStatus::Blocked(ThreadBlocker::ConditionVariable(waiting)) if waiting == id)
                {
        ...
        if matches!(
            ...
            ThreadStatus::Blocked(
                ThreadBlocker::Mutex(_)
                    | ThreadBlocker::ReacquireMutex(_)
                    | ThreadBlocker::ConditionVariable(_)
                    | ThreadBlocker::Thread(_)
            )
        ) {
            // A non-null wait_condvar also makes GNU's callback perform
            // post_acquire_global_lock for the signaling thread itself.
    ...
    fn check_thread_signal(&mut self) -> Result<(), LispError> {
        ...
        if let Some(thread) = self.find_thread_state_mut(self.active_thread_id)
            && !thread.signal_condition.is_nil()
        {
            let condition = std::mem::replace(&mut thread.signal_condition, Value::Nil);
            let data = std::mem::replace(&mut thread.signal_data, Value::Nil);
            return Err(LispError::SignalValue(build_signal_value(condition, data)));
        }
        Ok(())
    }
    ...
    fn thread_ready(&self, thread: &ThreadState, wake_sleepers: bool) -> bool {
        ...
        let signaled = !thread.signal_condition.is_nil();
        match thread.status {
