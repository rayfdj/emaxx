        ...
        match name {
            ...
            "rename-visited-file" => {
                ...
                let mut new_location =
                    resolve_file_name_in_env(interp, env, &string_text(&args[0])?);
            ...
            "normal-mode" => {
                ...
                if let Some((mode, dialect)) =
                    modes::interpreter_mode_function(interp, env, &source)?
                    && call_auto_mode(interp, &mode, env)?
                {
            ...
            "find-file-noselect" => {
                ...
                let remote_prefix =
                    parse_remote_file_name(&visited_name).map(|remote| remote.prefix);
                ...
                let result: Result<(), LispError> = (|| {
                    let mut mode =
                        modes::auto_mode_function_for_file_name(interp, env, &visited_name)?;
            ...
            "dired-revert" | "emaxx-dired-revert" => {
                ...
                let directory = interp
                    ...
                    .and_then(|value| string_like(&value).map(|string| string.text))
                    .ok_or_else(|| {
                        LispError::Signal("Current buffer is not a Dired buffer".into())
                    })?;
            ...
            "start-process" | "start-file-process" => {
                ...
                let program =
                    unquote_local_file_name(&requested_program).unwrap_or(requested_program);
                let argv = args[3..]
                    .iter()
                    .map(string_text)
                    .collect::<Result<Vec<_>, _>>()?;
            ...
            "process-live-p" => {
                ...
                let process_id = match &args[0] {
                    ...
                    value if string_like(value).is_some() => string_like(value)
                        .and_then(|name| interp.find_process_id_by_name(&name.text)),
                    _ => None,
                };
...
fn call_auto_mode(interp: &mut Interpreter, mode: &str, env: &mut Env) -> Result<bool, LispError> {
    let available = modes::handles(mode)
        || matches!(
            mode,
