...
editors. It rejects missing expectation evidence, inconsistent summaries,
incomplete/duplicate results, and unsuccessful child exits. Per-file
`execution_issues` and aggregate `unsuccessful_files` record execution health
separately from matching/mismatching outcome counts. Older resume records
without execution evidence are replayed rather than accepted as success.
...
The 16 compatibility-report tests passed, as did all 41 harness checks using
