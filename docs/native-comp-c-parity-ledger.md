...
| V02 | `SYMBOL_PLAINVAL`; `SYMBOL_VAL` | partial | Value cell, redirect and `declared_special`/`SYMBOL_LOCALIZED` flags live in one per-symbol cell (`SymbolCells`), indexed by a process-wide symbol id the `SymbolName` carries, so a symbol read is one index instead of three name-keyed probes; name-keyed callers reach the same cell through the interned table.  The cell also holds the plain value's native word, stamped with the heap and its collection generation; every value/alias/flag write clears it and a sweep retires the generation, so the process-wide value-cell epoch and the per-handle cache are gone.  Contracts: `variable_cells_follow_data_c`, five Rust-only cell tests in the anti-cheat audit, the two native GC tests that prove a collection retires cached words.  Still open: the cell is per interpreter rather than in the symbol object (the test image template is cloned across threads), and values that live outside the cell (forwarded C fields, buffer-local cells) still take the complete lookup path. |
| V03 | `SYMBOL_VARALIAS`; `SYMBOL_ALIAS`; `goto start` | partial | The redirect is stored in the alias's cell as the base symbol itself (not the end of the chain), and every read follows the chain, so re-pointing a base re-points its aliases; eval.c:Fdefvaralias is ported in order (CHECK_SYMBOL, constant, non-circularity from BASE signalling with BASE, built-in and buffer-local refusals, the 2008 value hand-over or the `losing-value` warning with its formatted message, let-bound refusal, watchers, both symbols declared special, LOADHIST_ATTACH, `variable-documentation` put even when nil, BASE returned); `Fsymbol_value`/`eval_sub` signal `void-variable` with the symbol given.  Contracts: `defvaralias_follows_eval_c`, `defvaralias_records_the_alias_in_load_history`.  Open: `trapped_write` is not modelled (an alias's own watchers are cleared rather than trapped to the base's), and the built-in refusal needs the forwarded manifest, which exists for the Linux oracle only. |
| V04 | `SYMBOL_LOCALIZED`; `SYMBOL_BLV`; `swap_in_symval_forwarding` | partial | A buffer's local bindings are an id-keyed table searched by the symbol object (`LocalCells`, GNU's `local_var_alist`), and the blv's `local_if_set`, the per-buffer slot kind and the always-local kind are flags in the symbol cell; a void local is a binding whose value is `Qunbound` (`local-variable-p` t, bare symbol in `buffer-local-variables`, void reads), which `makunbound`, `make-local-variable` of a void default, `set` through the cell, `specbind` over the cell and `buffer-local-value` now follow; `make-local-variable`/`make-variable-buffer-local` refuse constants and the latter turns a void plain value into nil and returns VARIABLE as given.  Contract: `buffer_local_cells_follow_data_c`.  Open: the blv's `where`/`valcell` swap (the current buffer's binding loaded into the symbol) is not represented -- every read still probes the current buffer's table -- and DEFVAR_PER_BUFFER slots keep Emaxx's auto-local model rather than `buffer_local_flags`. |
| V05 | `SYMBOL_FORWARDED`; `SYMBOL_FWD`; `do_symval_forwarding` | partial | Every name the contracted Linux oracle forwards carries `FORWARDED` in its symbol cell from interpreter construction, with `FWD_BOOL`/`FWD_INT` for the DEFVAR_BOOL/DEFVAR_INT kinds (source manifests with regeneration gates), so a store coerces to t/nil or is CHECK_INTEGER plus the intmax_t range check by kind rather than by name lookup, and `makunbound` clears the redirect as set_internal does; the 73 forwarded variables no Emaxx C owner defined (X11, Cairo, font tables, text conversion) are bound at construction to the oracle's `-Q --batch` default values from a manifest the anti-cheat gate re-prints from the oracle.  Contracts: `forwarded_c_variables_follow_store_symval_forwarding`, `oracle_only_forwarded_c_variables_are_bound_as_the_oracle_binds_them`.  Open: only eight C-owned Rust fields (quit-flag, inhibit-quit, throw-on-input, overriding-plist-environment, load-path, max-lisp-eval-depth, debug-on-next-call, symbols-with-pos-enabled) are read through the field itself; every other forwarded value lives in the cell, and the manifests exist for the Linux oracle only. |
