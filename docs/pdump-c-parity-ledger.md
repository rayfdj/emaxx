...
| D16 ENABLE | `emacs.c:main`, `init_environment`, `init_lread`, `after-pdump-load-hook`: initialize the current process around restored state; unchanged `startup.el` owns Lisp startup policy. | Implemented for supported images; integration validation in progress (2026-09-10). `load_pdump_at_startup` accepts explicit `--dump-file` or an executable sibling image, with GNU fatal error categories. Restored startup skips loadup, retains delayed Custom and native load-path state, and reapplies the process environment snapshot, argv, exec-path, TZ, message reset, C load-path initialization and frame faces. Safe hooks bind inhibit-quit, establish a signal handler before callbacks and propagate nonlocal exits; interactive hooks follow terminal setup. Open: D14/D15 native images, installed `PATH_EXEC` image discovery and `--temacs`. | The batch and CLI `image_round_trip_or_explicit_native_image_limit` controls distinguish a supported restore from the known native refusal. The positive `tools/dumped_startup_gate.py` accepts only real image startup and all three original timeout tests passing in both editors. See [the integration audit](native-loader-integration-audit-2026-09-10.md); no fresh frozen score yet. |
