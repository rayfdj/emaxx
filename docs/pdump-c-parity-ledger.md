...
| D09 BUILD | `pdumper.c:dump_symbol`, `dump_string`, `dump_vectorlike_generic`, `dump_obarray`, numeric and interval writers: save supported type-specific state. | Partial (2026-09-08): conses, immutable and mutable strings with their property spans (the interval tree's content), symbols (flags, name, value or alias, function, plist, watchers), ordinary vectors, floats, bignums and integers beyond the fixnum range, built-in functions are written and read back; the obarray record is written (reading it back waits for an interpreter-root image, D12).  Not written yet: the plist and function cell of `nil` and `t` (self-representing words here, copied built-in symbols in GNU), and the interned-in-another-obarray state (every interned symbol is recorded as interned in the initial obarray).  Not yet: closures and their lexical environments, char-tables, records and pseudovectors, bool-vectors, buffers, markers, overlays, finalizers, reader forms -- `dump_object` refuses each with pdumper.c's `unsupported object type in dump: KIND` (referrer paths printed when TRACK-REFERRERS is non-nil), so a real `dump-emacs-portable` stops at the first such object (a record, in the current image). | Cover each type reached from real loadup, including closure constants/environments and mutable/propertized strings; do not silently omit types. |
